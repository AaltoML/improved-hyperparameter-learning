...
def broadcasting_ABT(A, B):
    """
    given A = [a1, a2, ..., aN] #[M, N]
          B = [b1, b2, ..., bN] #[M, N]

    return [a1b1^T, a2b2^T, ..., aNbN^T] #[N, M, M]
    """

    A = tf.expand_dims(tf.transpose(A), 2)  # [N, M, 1]
    B = tf.expand_dims(tf.transpose(B), 2)  # [N, M, 1]

    return tf.matmul(A, B, transpose_b=True)  # [N, M, M]


def broadcasting_ATB(A, B):
    """
    given A = [a1, a2, ..., aN] #[M, N]
          B = [b1, b2, ..., bN] #[M, N]

    return [a1^Tb1, a2^Tb2, ..., aN^TbN] #[N, 1, 1]
    """

    A = tf.expand_dims(tf.transpose(A), 1)  # [N, M, 1]
    B = tf.expand_dims(tf.transpose(B), 1)  # [N, M, 1]

    return tf.matmul(A, B, transpose_b=True)  # [N, 1, 1]
...
def conditional_from_precision_sites(
    ...
    mean = tf.matmul(Kuf, l, transpose_a=True) - tf.linalg.matrix_transpose(
        tf.reduce_sum(
            tf.matmul(D, tf.matmul(Kuu, tf.linalg.matrix_transpose(l)[..., None]))
            * tmp,
            axis=-2,
        )
    )
