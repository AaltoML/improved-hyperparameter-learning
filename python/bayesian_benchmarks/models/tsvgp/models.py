...
class ClassificationModel:
    ...
    def fit(self, X, Y, ARD):
        ...
        for _ in trange(self.ARGS.iterations):
            ...
            for _ in range(20):
                self.opt.minimize(
                    lambda: -self.model.elbo(data), self.model.trainable_variables
                )
        ...
        result = {
            "len_trajectory": self.model.kernel.lengthscales.numpy(),
            "var_trajectory": self.model.kernel.variance.numpy(),
            "total_elbo": self.model.elbo(data).numpy(),
            "total_ep": self.model.ep_estimation(data).numpy(),
            "stopped": stopped,
        }
