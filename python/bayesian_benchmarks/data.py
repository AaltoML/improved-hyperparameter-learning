...
class Dataset(object):
    ...
    def datapath(self):
        filename = self.url.split("/")[
            -1
        ]  # this is for the simple case with no zipped files
...
class Classification(Dataset):
    ...
    def needs_download(self):
        if os.path.isfile(
            os.path.join(DATA_PATH, "classification_data", "iris", "iris_R.dat")
        ):
            return False
        else:
            return True
    ...
    def download(self):
        logging.info(
            "donwloading classification data. WARNING: downloading 195MB file".format(
                self.name
            )
        )
    ...
    def read_data(self):
        datapath = os.path.join(
            DATA_PATH, "classification_data", self.name, self.name + "_R.dat"
        )
        if os.path.isfile(datapath):
            data = np.array(
                pandas.read_csv(datapath, header=0, delimiter="\t").values
            ).astype(float)
        else:
            data_path1 = os.path.join(
                DATA_PATH, "classification_data", self.name, self.name + "_train_R.dat"
            )
            data1 = np.array(
                pandas.read_csv(data_path1, header=0, delimiter="\t").values
            ).astype(float)

            data_path2 = os.path.join(
                DATA_PATH, "classification_data", self.name, self.name + "_test_R.dat"
            )
            data2 = np.array(
                pandas.read_csv(data_path2, header=0, delimiter="\t").values
            ).astype(float)
...
urls_mujoco_dataset = {
    "Ant-v2": "https://drive.google.com/uc?export=download&id=1NdoF79nOg53_fjTlth4iN1RHpQRYgYw0",
...
class MujocoSoftActorCriticDataset(Dataset):
    ...
    def _read_checkpoint_data(self, evaluation_file):
        ...
        for evaluation in range(evaluations):
            ...
            delta_observation_trajec = (
                observation_trajec[1:, :] - observation_trajec[:-1, :]
            )
...
class Humanoid(MujocoSoftActorCriticDataset):
    ...
    def download(self):
        for i in range(1, 6):
            ...
            with urlopen(self.url_mujoco_sub_datasets[str(i)]) as response, open(
                filename, "wb"
            ) as out_file:
                data = response.read()
                out_file.write(data)
        ...
        for i in range(1, 6):
            dirname = os.path.join(
                self.datadir + "-" + str(i), "env=" + self.name + "-" + str(i)
            )
