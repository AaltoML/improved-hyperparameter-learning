...
class BatchNorm(tfb.Bijector):
    ...
    def _forward(self, u):
        if not self._vars_created:
            self._create_vars(u)
        return (u - self.beta) * tf.exp(-self.gamma) * tf.sqrt(
            self.train_v + self.eps
        ) + self.train_m
    ...
    def _inverse(self, x):
        ...
        update_train_m = tf.assign_sub(self.train_m, self.decay * (self.train_m - m))
        update_train_v = tf.assign_sub(self.train_v, self.decay * (self.train_v - v))
        ...
        with tf.control_dependencies([update_train_m, update_train_v]):
            return (x - m) * 1.0 / tf.sqrt(v + self.eps) * tf.exp(
                self.gamma
            ) + self.beta
    ...
    def _inverse_log_det_jacobian(self, x):
        ...
        abs_log_det_J_inv = tf.reduce_sum(self.gamma - 0.5 * tf.log(v + self.eps))
    ...
    def make_flow(self):
        ...
        for i in range(num_layers):
            with tf.variable_scope("bijector_%d" % i):
                ...
                alpha = tf.abs(tf.get_variable("alpha", [], dtype=DTYPE)) + 0.01
...
sess = tf.Session()
model = FancyModel("IAF", data.X_train[:, :2].astype(np.float32), batchsize=5000)
